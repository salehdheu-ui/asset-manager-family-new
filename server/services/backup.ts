...
export async function createBackupSnapshot(createdBy?: string | null, backupLevel: BackupLevel = "snapshot") {
  ...
  const [settingsRows, memberRows, contributionRows, loanRows, repaymentRows, paymentRows, expenseRows, adjustmentRows, allocationRows, userRows, auditLogRows, zakatRows, investmentRows, valuationRows, proposalRows, proposalVoteRows, attachmentRows, rateRows, loanVoteRows] = await Promise.all([
